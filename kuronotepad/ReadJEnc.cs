...
namespace Hnx8.ReadJEnc {
    ...
    public class ReadJEnc {   ////////////////////////////////////////////////////////////////////////
        ...
        public CharCode GetEncoding(byte[] bytes, int len, out string text) {
            ...
            {
                if (b1 <= BINARY) {   //バイナリ文字検出：先頭２バイトでの検出ならUTF16Nの可能性をチェック、否ならバイナリ確定
                    CharCode ret = (asciiEndPos < 2 ? SeemsUTF16N(bytes, len) : null);
                    if (ret != null && (text = ret.GetString(bytes, len)) != null) {   //UTF16Nデコード成功：非テキスト文字混入チェック
                        int i;
                        for (i = -3; i <= BINARY; i++) {   //0xFFFD,0xFFFE,0xFFFF,0～BINARY、DELが混入している場合は非テキストとみなす
                            if (text.IndexOf((char)i, 0, text.Length) != -1) { break; }
                        }
                        if (i > BINARY && text.IndexOf((char)DEL, 0, text.Length) == -1) {   //■UTF16N確定（非テキスト文字混入なし）
                            return ret;
                        }
                    }
                    text = null;
                ...
                if (b1 == 0x1B) {   //エスケープシーケンス判定(エスケープコード内容を読み飛ばす)
                ...
                //次の文字へ
                if ((++asciiEndPos) >= len) {   //全文字チェック完了：非ASCII文字未検出、JISもしくはASCII
                    if (escapeSequenceChecker != null) {   //エスケープシーケンスに基づく文字コードが取得できるか確認
                        CharCode ret = escapeSequenceChecker.GetEncoding(out text);
                        if (ret != null) {   //■エスケープシーケンスに基づく文字コードで確定
                            return ret;
                        }
                    }
                    else if (JIS.HasSOSI(bytes, len)) {   //SO,SIによるエスケープを検出した場合は、半角カナJISの可能性を判定
                        if (escapeSequenceChecker == null && (text = CharCode.JIS50222.GetString(bytes, len)) != null) {   //■エスケープシーケンスなし、半角カナSOSIのみを使用したJISで確定
            ...
            {
                if (b1 == DEL) {   //制御文字0x7F登場なら、ごくわずかなJISの可能性以外全消滅。JISの可能性を消しきれるか判定
                    cp1252Score = int.MinValue;
                    utfScore = int.MinValue;
                    eucScore = int.MinValue;
                    sjisScore = int.MinValue;
                    if (escapeSequenceChecker == null || (cp1252Pos++) >= len || (b1 = bytes[cp1252Pos]) < 0x21 || b1 >= DEL) {   //JISエスケープ未出現 or ファイル末尾で2バイト目なし or 2バイト目が0x21-0x7E範囲外ならJISの可能性も否定
                        text = null;
                        return null; //■バイナリ確定
                    }
                }
                ...
                switch (cp1252Score) {
                    ...
                    default: //CP1252可能性あり、定義外文字混入チェック＆CP1252ポイント加算
                        while (b1 > DEL) {   // 非CP1252チェック用定義(0x2001A002)：未定義の81,8D,8F,90,9Dに対応するビットがON
                            //        FEDC BA98 7654 3210         FEDC BA98 7654 3210
                            //        ---- ---- ---- ----         ---- ---- ---- ----
                            // (0x9#) 0010 0000 0000 0001  (0x8#) 1010 0000 0000 0010
                            if (b1 <= 0x9D && (0x2001A002 & (1u << (b1 % 32))) != 0) {   // CP1252未定義文字を検出、可能性消滅
                                cp1252Score = int.MinValue;
                        ...
                        else if (cp1252Pos == notAsciiStart + 2 && (b2 = bytes[cp1252Pos - 1]) >= 0xC0) {   //２バイトのみ出現時、ダイアクリティカルマーク（発音記号等）つきアルファベットなら配点補正
                            if (b2 == (b2 = bytes[cp1252Pos - 2])) { cp1252Score += 5; } //同一文字重ねはかなり特徴的(SJISカナより可能性高)
                            else if (b2 >= 0xC0) {   //続きor直前のASCII文字がアルファベットっぽければ、SJISカナより可能性が高くなるよう補正                               
                ...
                if (utfScore >= 0) {
                    ...
                    for (int utfPos = notAsciiStart; utfPos < cp1252Pos; utfPos++) {
                        ...
                        else if (b1 < 0xE0) {   //２バイト文字OK（半角文字とみなして評価）
                ...
                if (eucScore >= 0) {   //前の文字との連続性チェック用定数定義
                    const int PREV_KANA = 1; //直前文字は半角カナ
                    const int PREV_ZENKAKU = 2; //直前文字は全角
                    int prevChar = 0; //前の文字はKANAでもZENKAKUでもない
                    for (int eucPos = notAsciiStart; eucPos < cp1252Pos; eucPos++) {   //１バイト目(0xA1-0xFE,0x8E,0x8F)・２バイト目(１バイト目に応じ範囲が異なる)のチェック
                        b1 = bytes[eucPos];
                        if (b1 == 0xFF || (++eucPos) >= cp1252Pos) { eucScore = int.MinValue; break; } //EUC可能性消滅
                        b2 = bytes[eucPos];
                        if (b1 >= 0xA1) {   //１バイト目＝全角文字指定、２バイト全角文字チェック
                        ...
                        else if (b1 == 0x8E) {   //１バイト目＝EUC-JPのかな文字(orEUC-TWの４バイト文字)指定。２バイトの半角カナ文字チェック
                            ...
                            && b2 >= 0xA1 && b2 < 0xFF
                            && (++eucPos) < cp1252Pos
                            && (b2 = bytes[eucPos]) >= 0xA1 && b2 < 0xFF) {   //残る可能性は３バイト文字：検出OKならEUC文字数を加算（全角文字、補助漢字）
            ...
            if (escapeSequenceChecker != null) {   //JIS系可能性高：エスケープシーケンスに基づく文字コードが取得できるか確認
                CharCode ret = escapeSequenceChecker.GetEncoding(out text);
                if (ret != null) { return ret; } //■エスケープシーケンスに基づく文字コードで確定
            }
            if (eucScore > 0 && eucScore > sjisScore && eucScore > utfScore) {   //EUC可能性高
                if (cp1252Score > eucScore) {   //ただしCP1252の可能性が高ければCP1252を先にチェック
            ...
            if (utfScore > 0 && utfScore >= sjisScore) {   //UTF可能性高
                if ((text = CharCode.UTF8N.GetString(bytes, len)) != null) { return CharCode.UTF8N; } //■UTF-8Nで読みこみ成功
            }
            if (sjisScore >= 0) {   //SJISなどの各国語指定に合致したなら、そのコードでの読み出しを試みる(ただしCP1252の可能性が高ければCP1252を先にチェック)
            ...
            if (cp1252Score > 0) {   //CP1252の可能性のみ残っているのでチェック
                if ((text = CharCode.ANSI.GetString(bytes, len)) != null) { return CharCode.ANSI; } //■CP1252で読みこみ成功
            }
        ...
        protected virtual int GetEncoding(byte[] bytes, int pos, int len) => int.MinValue;
        ...
        public static CharCode SeemsUTF16N(byte[] bytes, int len) {
            if (len >= 2 && len % 2 == 0) {
                if (bytes[0] == 0x00) {
                    if (bytes[1] > BINARY && bytes[1] < DEL && (len == 2 || bytes[2] == 0)) {   //▲UTF16BigEndianの可能性あり
                        return CharCode.UTF16BE;
                    }
                }
                else if (bytes[1] == 0x00) {
                    if (bytes[0] > BINARY && bytes[0] < DEL && (len == 2 || bytes[3] == 0)) {   //▲UTF16LittleEndianの可能性あり
        ...
        public override string ToString() => CharCode.Name;
        #endregion
        ...
        private class JIS {
            ...
            internal static bool HasSOSI(byte[] bytes, int len) => (Array.IndexOf<byte>(bytes, 0x0E, 0, len) >= 0 && Array.IndexOf<byte>(bytes, 0x0F, 0, len) >= 0);
            ...
            internal int GetEncoding(int pos) {
                if (pos + 2 < len) {   //ESC(0x1B):有効なエスケープシーケンスかチェック
                    c++; //加算前提
                    switch (bytes[pos + 1]) {   //２バイト目で分岐
                        case 0x24: //ESC$
                            switch (bytes[pos + 2]) {   //３バイト目で分岐
            ...
            internal CharCode GetEncoding(out string text) {
                ...
                int len = this.len;
                if (ISOKR && HasSOSI(bytes, len)) {   //■KSエスケープシーケンスあり、ISO-2022-KRで確定(半角カナJISではない)
                    text = CharCode.ISOKR.GetString(bytes, len);
                    return (text != null ? CharCode.ISOKR : null);
                }
                if (c <= 0) {   //JIS評価値がマイナスないしゼロならばJISではないと判断
                    text = null;
                    return null;
                }

                if (JISH) {   //補助漢字のエスケープシーケンスあり、補助漢字を考慮してデコードする
                    try {
                        StringBuilder ret = new StringBuilder(len);
                        int pos = 0;
                        while (pos < len) {   //JIS補助漢字エスケープ以外の範囲を把握
                            int start = pos;
                            while (pos < len) {
                                if (bytes[pos] == 0x1B && pos + 3 < len &&
                                    bytes[pos + 1] == 0x24 &&
                                    bytes[pos + 2] == 0x28 &&
                                    bytes[pos + 3] == 0x44) {   //JIS補助漢字エスケープシーケンスを検出、ループ脱出
                                    break;
                                }
                                pos++;
                            }
                            if (start < pos) {   //通常のCP5022Xでデコードする
                                ret.Append(CharCode.JIS.GetEncoding().GetString(bytes, start, pos - start));
                            }
                            //JIS補助漢字エスケープ部分の処理
                            if (pos < len) {   //JIS補助漢字エスケープシーケンス除去、補助漢字範囲特定
                                pos = pos + 4;
                                start = pos;
                                while (pos < len && bytes[pos] != 0x1B) { pos++; }
                                if (start < pos) {
                                    byte[] bytesForCP20932 = new byte[pos - start];
                                    for (int i = 0; i < bytesForCP20932.Length; i++) {   //CP20932のコード体系に合わせ、１バイト目は0xA1-0XFE,２バイト目は0x21-0x7Eとなるようにする
                    ...
                    catch (DecoderFallbackException) {   //読み出し失敗(マッピングされていない文字があった場合など)
                        text = null;
                    }
                }
                else {   //補助漢字なしJISとしてデコードする
                    text = CharCode.JIS.GetString(bytes, len);
                }

                if (text == null) {   //■デコード不能箇所有、JIS破損ファイルとみなす
        ...
        private class SJIS : ReadJEnc {
            ...
            protected override int GetEncoding(byte[] bytes, int pos, int len) {
                ...
                byte b1 = bytes[pos];
                byte b2;
                while (pos < len) {   //前の文字との連続性チェック用定数定義
                    const int PREV_KANA = 1; //直前文字は半角カナ
                    const int PREV_ZENKAKU = 2; //直前文字は全角
                    int prevChar = 0; //前の文字はKANAでもZENKAKUでもない
                    while (b1 > DEL) {
                        if (b1 >= 0xA1 && b1 <= 0xDF) {   //１バイト半角カナ：OK（連続はEUCやCP1252よりも高配点とする）
                        ...
                        // (0xF#) 1110 0000 0000 0000  (0xE#) 1001 1000 0000 0000 - FD,FE,FF:定義外、EB,EC,EF:未使用 (F0-F9:外字は許容。HNXgrepなど外字不許容とする場合はビットを立てること)
                            ...
                            || (++pos) >= len
                            || (b2 = bytes[pos]) < 0x40 || b2 > 0xFC) {   //１バイト目がおかしい(SJIS定義外/未使用) or ２バイト目把握不能 or ２バイト目SJIS定義外
                            return int.MinValue; //可能性消滅
                        }
                        else {   //全角文字数を加算(EUCよりは可能性を低めに見積もっておく)
                            if (prevChar == PREV_ZENKAKU) { score += 4; }
                            else {   //（ただし唐突に0x98以降の第二水準文字が出てきた場合は、UTF-8/EUC/CP1252の可能性が高いのでプラス値なしとする）
                                score += (b1 > 0x98 ? 0 : 2);
        ...
        private class BIG5TW : ReadJEnc {
            internal BIG5TW() : base(CharCode.BIG5TW, CharCode.EUCTW) => EUCTW = true;

            protected override int GetEncoding(byte[] bytes, int pos, int len) {   //※配点未調整（EUC/UTF8はおそらく正しく識別可能、CP1252は一部誤判別する可能性あり）
                int score = 0; //初期値ゼロからReadJEnc評価を始める
                byte b1 = bytes[pos];
                byte b2;
                while (pos < len) {   //前の文字との連続性チェック用定数定義
                    ...
                    while (b1 > DEL) {
                        if (b1 < 0x81 || b1 > 0xF9 || b1 == 0xC7 || b1 == 0xC8
                            || (++pos) >= len
                            || (b2 = bytes[pos]) < 0x40 || (b2 < 0xA1 && b2 > 0x7E) || b2 > 0xFE) {   //１バイト目がBig5定義外 or ２バイト目把握不能 or ２バイト目がBig5定義外
                            return int.MinValue; //可能性消滅
                        }
                        else {   //全角文字数を加算(EUCよりは可能性を低めに見積もっておく)
                            if (prevChar == PREV_ZENKAKU) { score += 4; }
                            else {   //ただし唐突に外字・次常用字(第二水準)が出てきた場合は配点を低めにする
        ...
        private class GB18030 : ReadJEnc {
            ...
            protected override int GetEncoding(byte[] bytes, int pos, int len) {   //※配点未調整（EUC/UTF8はおそらく正しく識別可能、CP1252は誤判別の可能性大）
                int score = 0; //初期値ゼロからReadJEnc評価を始める
                byte b1 = bytes[pos];
                byte b2;
                while (pos < len) {   //前の文字との連続性チェック用定数定義
                    ...
                    int prevChar = 0; //前の文字はKANAでもZENKAKUでもない
                    while (b1 > DEL) {
                        if (b1 < 0x81 || b1 > 0xFE || (++pos) >= len) {   //１バイト目がおかしい(GB定義外) or ２バイト目把握不能
                            return int.MinValue; //可能性消滅
                        }
                        else if ((b2 = bytes[pos]) >= 0x40 && b2 <= 0xFE) {   //２バイト文字：全角文字数を加算(EUCよりは可能性を低めに見積もっておく)
                            ...
                            && (++pos) < len
                            && (b2 = bytes[pos]) >= 0x30 && b2 <= 0x39) {   //４バイト全角文字：全角文字数を加算（かなりGB18030の決め手になる特徴なので、ポイントを高めに）
            ...
            protected override int GetEncoding(byte[] bytes, int pos, int len) {   //※配点未調整（EUC/UTF8はおそらく正しく識別可能、CP1252は誤判別の可能性大）
                ...
                while (pos < len) {   //前の文字との連続性チェック用定数定義
                    ...
                    while (b1 > DEL) {
                        if (b1 < 0x81 || b1 > 0xFE
                            || (++pos) >= len
                            || (b2 = bytes[pos]) < 0x41 || (b2 < 0x61 && b2 > 0x5A) || (b2 < 0x81 && b2 > 0x7A) || b2 > 0xFE) {   //１バイト目がおかしい(UHC定義外) or ２バイト目把握不能 or ２バイト目がUHC定義外
                            return int.MinValue; //可能性消滅
                        }
                        else {   //全角文字数を加算(EUCよりは可能性を低めに見積もっておく)
                            if (prevChar == PREV_ZENKAKU) { score += 4; } else { score += 2; prevChar = PREV_ZENKAKU; }
