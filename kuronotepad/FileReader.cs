...
namespace Hnx8.ReadJEnc {
    ...
    public class FileReader : IDisposable {   ////////////////////////////////////////////////////////////////////////
        // <FileReader.cs> ReadJEnc File読出＆文字コード自動判別(Rev.20150309)
        ...
        public FileReader(int len) => Bytes = new byte[len];
        ...
        public virtual CharCode Read(FileInfo file) {
            Length = 0;
            text = null;
            try {   //無用なDiskIOを極力行わないよう、オープン前にもファイルサイズチェック
                ...
                using (FileStream stream = file.Open(FileMode.Open, FileAccess.Read, FileShare.ReadWrite)) {   //オープン後の実際のファイルサイズによるチェック
                    long filesize = stream.Length;
                    if (filesize == 0) { return FileType.EMPTYFILE; } //■空ファイル
                    if (filesize > Bytes.Length) { return FileType.HUGEFILE; } //■巨大ファイル
                    if (filesize > 65536) {   //一定サイズ以上の大きいファイルなら、BOM/マジックナンバー判定に必要な先頭バイトを読み込み、判断
                        Length = stream.Read(Bytes, 0, FileType.GetBinaryType_LEASTREADSIZE);
                        c = GetPreamble(filesize);
                        if (c == null || c is CharCode.Text) {   //残りの読みこみ（ただし非テキストと確定した場合は省略）
                            Length += stream.Read(Bytes, this.Length, (int)filesize - this.Length);
                        }
                    }
                    else {   //大きくないファイルは一括で全バイト読み込み、判断
                        Length = stream.Read(Bytes, 0, (int)filesize);
                        c = GetPreamble(filesize);
                    }
                }
                if (c is CharCode.Text) {   //BOMありテキストなら文字列を取り出す（取り出せなかったら非テキスト扱い）
                    if ((text = c.GetString(Bytes, Length)) == null) { c = null; }
                }
                else if (c == null) {   //ファイル文字コード種類不明なら、全バイト走査して文字コード確定
        ...
        protected virtual CharCode GetPreamble(long len) {
            ...
            if (ret == null && Array.IndexOf<byte>(this.Bytes, 0x00, 0, this.Length) >= 0) {   //UTF16Nの可能性がなければバイナリファイルとみなす
                if (ReadJEnc.SeemsUTF16N(this.Bytes, (int)len) == null) {   //■バイナリ確定（マジックナンバーからファイル種類を決定）
